-- Aggregate existing sale item quantities by product before applying sale-change inventory deltas.
-- This prevents duplicate inventory_event_lines rows for historical sales that contain
-- multiple sale_items rows for the same product_id.
  ...
  if p_action_type = 'edit_sale' then
    ...
    for v_delta in
      ...
      select
        inventory.product_id,
        inventory.stock as previous_stock,
        inventory.stock + existing.quantity as resulting_stock
      from existing
      join public.booth_schedule_products as inventory
        on inventory.schedule_id = v_sale.schedule_id
       and inventory.product_id = existing.product_id
      order by inventory.product_id
      for update of inventory
    loop
      v_locked_row_count := v_locked_row_count + 1;
